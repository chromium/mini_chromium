...
def GetProvisioningProfilesDir():
    ...
    """
    return os.path.join(os.environ['HOME'], 'Library', 'MobileDevice',
...
def ReadPlistFromString(plist_bytes):
    ...
    Args:
        plist_bytes: contents of property list to load. Must be bytes in python
        3.
...
class Bundle(object):
    ...
    def __init__(self, bundle_path, platform):
        """Initializes the Bundle object with data from bundle Info.plist
        file."""
    ...
    def Kind(platform, extension):
        ...
        if platform == 'macosx':
            ...
            return 'mac'
        raise ValueError('unknown bundle type %s for %s' %
                         (extension, platform))
    ...
    def Validate(self, expected_mappings):
        ...
        Args:
            expected_mappings: a dictionary of string to object, each mapping
            will be looked up in the bundle data to check it has the same value
            (missing values will be ignored)

        Returns:
            A dictionary of the key with a different value between
            expected_mappings and the content of the bundle (i.e. errors) so
            that caller can format the error message. The dictionary will be
...
class ProvisioningProfile(object):
    ...
    def ValidToSignBundle(self, bundle_identifier):
        ...
        Args:
            bundle_identifier: the identifier of the bundle that needs to be
            signed.
...
def FindProvisioningProfile(bundle_identifier, required):
    ...
    selected_provisioning_profile = max(
        valid_provisioning_profiles,
        key=lambda p:
        (len(p.application_identifier_pattern), p.expiration_date))
...
def InstallSystemFramework(framework_path, bundle_path, args):
    """Install framework from |framework_path| to |bundle| and code-re-sign
    it."""
    installed_framework_path = os.path.join(bundle_path, 'Frameworks',
                                            os.path.basename(framework_path))
    ...
    CodeSignBundle(
        installed_framework_path, args.identity,
...
class CodeSignBundleAction(Action):
    ...
    def _Register(parser):
        ...
        parser.add_argument('--disable-code-signature',
                            action='store_true',
                            dest='no_signature',
        ...
        parser.add_argument(
            ...
            default=[],
            help=
            'path to partial Info.plist to merge to create bundle Info.plist')
        parser.add_argument(
            '--plist-compiler-path',
            '-P',
            action='store',
    ...
    def _Execute(args):
        ...
        # The bundle Info.plist may have been updated by
        # GenerateBundleInfoPlist() above. Load the bundle information from
        # Info.plist after the modification have been written to disk.
        bundle.Load()

        # According to Apple documentation, the application binary must be the
        # same as the bundle name without the .app suffix. See crbug.com/740476
        # for more information on what problem this can cause.
        #
        # To prevent this class of error, fail with an error if the binary name
        # is incorrect in the Info.plist as it is not possible to update the
        # value in Info.plist at this point (the file has been copied by a
        # different target and ninja would consider the build dirty if it was
        # updated).
        #
        # Also checks that the name of the bundle is correct too (does not cause
        # the build to be considered dirty, but still terminate the script in
        # case of an incorrect bundle name).
        #
        ...
        if args.embedded_mobileprovision:
            # Find mobile provisioning profile and embeds it into the bundle (if
            # a code signing identify has been provided, fails if no valid
            # mobile provisioning is found).
            ...
            if provisioning_profile and args.platform != 'iphonesimulator':
                ...
                if args.entitlements_path is not None:
                    ...
                    entitlements = GenerateEntitlements(args.entitlements_path,
...
class GenerateEntitlementsAction(Action):
    ...
    def _Register(parser):
        ...
        parser.add_argument('path',
                            help='path to the entitlements file to generate')
